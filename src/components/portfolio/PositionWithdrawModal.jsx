...
export default function PositionWithdrawModal({position, onClose = () => {}}) {
  ...
  const newHealthFactorQuery = useGetHypotheticalHealthFactor({
    token: position,
    collAmount: amount,
    collIncrease: false
  });
  ...
  const handleWithdraw = () => {
    ...
    if (createBigNumber(amount.toString()).gt(maxWithdrawAbleQuery.data?.toString() || "0"))
      return new ValidationException(
        "Withdraw Amount Is Greater than Available To Withdraw Amount"
      );
  ...
  const remainingSize = useMemo(() => {
    ...
    if (amount && parseFloat(amount) !== 0) wantToWithdraw = amount;
  ...
  const buttonDisabledReason = useMemo(() => {
    ...
    if (maxWithdrawAbleQuery.isLoading) return "Calculating Max Withdrawal...";

    if (createBigNumber(amount.toString()).gt(maxWithdrawAbleQuery.data.toString() || "0"))
      return "Requested Amount Is Greater Than Max Allowed";

    if (newHealthFactorQuery.isLoading) return "Calculating Health Factor...";
    ...
    >
      <Modal.Body className="p-4 radius-8 bg-trans1 border-dark w-100">
        ...
        <div className="mt-4 mb-4 loop-info">
          ...
          <div className="mt-2">
            Withdrawing collateral with an active borrow position can impact Health Factor. See the
            Details section to make sure your Health Factor remains above 100%
          </div>
        </div>
          ...
          >
            ...
            <div className="text-end">
              <Skeleton loading={size === env.EMPTY_VALUE} width="80px">
                <div className="font-16 bold-700 color-white my-2">{size}</div>
              </Skeleton>
            </div>
          </div>
        ...
        <Skeleton
          loading={position.userPoolBalance === env.EMPTY_VALUE || maxWithdrawAbleQuery.isLoading}
        >
          <div
        ...
        <div className="d-flex v-center space-between mt-2">
          ...
          <div className="text-end">
            <Skeleton loading={remainingSize === env.EMPTY_VALUE} width="80px">
              <div className="font-16 bold-700 color-white my-2">{remainingSize}</div>
            </Skeleton>
          </div>
        </div>
