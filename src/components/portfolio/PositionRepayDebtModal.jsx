...
export default function PositionRepayDebtModal({ position, onClose = () => { } }) {
  ...
  const handleRepay = () => {
    ...
    if (!amount)
      return new ValidationException('Fill Repay Amount First')
  ...
  const debt = useMemo(() => {
    ...
    if (position.userBorrows === env.EMPTY_VALUE)
  ...
  const buttonDisabledReason = useMemo(() => {
    ...
    if (!amount)
      return 'Fill Repay Amount First'
      ...
      <Modal.Body className='p-4 radius-8 bg-trans1 border-dark w-100'>
        ...
        <div className='radius-8 border-2 border-dark-green p-3 mt-4'>
          <div className='d-flex space-between v-center'>
            ...
            <div className='radius-8 bg-trans px-3 py-2 font-14 bold-700 color-white cursur-pointer' onClick={() => setRepayDetailVisible(!repayDetailVisible)}>{repayDetailVisible ? 'Hide' : 'Show'}</div>
          </div>
          <div className={`d-flex v-center space-between mt-2 ${!repayDetailVisible ? "d-none" : ""}`}>
            <div>
        ...
        <div className='d-flex v-center mt-4 mb-2'>
          ...
          <div className='w-100 d-flex justify-content-between align-items-center'>
            ...
            <Skeleton loading={balanceQuery.isLoading || balanceQuery.isFetching} width='100px' height='20px'>
              <span className='font-14 bold-400 color-gray'>{`Balance: ${truncateAmount(balanceQuery?.data?.toString())} ${position.name || ''}`}</span>
            </Skeleton>
          </div>
        </div>
        ...
        <div className='mt-2'>
          <AsyncButton onClick={handleRepay} loading={buttonLoading} disabledreason={buttonDisabledReason} className='mt-4'>Repay Debt</AsyncButton>
        </div>
      </Modal.Body>
    </Modal >
  )
