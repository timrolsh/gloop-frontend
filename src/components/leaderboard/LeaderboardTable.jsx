        ...
        <div className="radius-8 bg-trans p-4 ">
            <div className='desktop-show'>
                <Table borderless className='gmi_basket_table'>
                    ...
                    <tbody>
                        <tr><td colSpan={6}><hr className='hr-3 border-dark-green m-0' /></td></tr>
                        {
                            !isLoading && !rankings.length
                                ? <tr><td className='color-white text-center' colSpan={8}>
                                    <span>{isError ? 'Failed To Fetch Leaderboard Data' : 'No Leaderboard Data Available'}</span>
                                </td></tr>
                                : rankings.length && rankings.map((ranking, index) => (
            ...
            <div className='mobile-show'>
                {
                    !isLoading && !rankings.length
                        ? <span className='color-white text-center w-100 d-block'>{isError ? 'Failed To Fetch Leaderboard Data' : 'No Leaderboard Data Available'}</span>
                        : rankings.length && rankings.map((ranking, index) => (
