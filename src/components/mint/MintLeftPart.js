      ...
      <div className="d-flex v-center space-between">
        <div className="w-100">
          <div
            ...
            onClick={() => setActiveTab("Mint GDC")}
          >
            Mint GDC
          </div>
          <hr
            className={`hr-3 border-dark-green ${activeTab == "Mint GDC" ? "active_gmi_tab" : ""}`}
          />
        </div>
        <div className="w-100">
          <div
            className="font-18 bold-600 color-white text-center cursur-pointer py-2"
            onClick={() => setActiveTab("Redeem GDC")}
          >
            Redeem GDC
          </div>
          <hr
            className={`hr-3 border-dark-green ${
              activeTab == "Redeem GDC" ? "active_gmi_tab" : ""
            }`}
          />
        </div>
      </div>
      {activeTab == "Mint GDC" ? <MintGdc /> : <RedeemGdc />}
    </div>
  );
}
