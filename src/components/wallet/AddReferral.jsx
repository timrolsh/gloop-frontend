...
export default function AddReferral({onClose = () => {}}) {
  ...
  const redeemButtonDisabledReason = useMemo(() => {
    if (!referralWalletAddress) return "Please Fill Referrer’s Wallet Address";

    if (referralWalletAddress.length < 32)
      return "Referrer’s Wallet Address Should Be Atleast 32 Characters";

    if (referralWalletAddress === userWalletAddress) return `Address can't Be It's Own Referrer`;
  ...
  return (
    <Modal show={true} onHide={onClose} centered className="buy_gloop_modal">
      <Modal.Body className="px-3 radius-8 bg-trans1 border-dark w-100" style={{height: "340px"}}>
        <div>
          <img src={close_img_url} width={10} className="invite_modal_close" onClick={onClose} />
        </div>

        <div className="d-flex flex-column space-between h-100">
          <div className="font-18 bold-500 color-white text-center">
            Enter Referrer’s Wallet Address
          </div>
          ...
          <div className=" loop-info">
            ...
            <div className="mt-2">
              Only one referrer can be set per address and is then forever locked. An address cannot
              be its own referrer.
            </div>
          </div>
          ...
          <div
            ...
            className="radius-8 bg-trans desktop-flex space-between v-center"
          >
            <input
              type="text"
