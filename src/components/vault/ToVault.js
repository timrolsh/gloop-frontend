...
export default function ToVault() {
  ...
  window.addEventListener("click", (e) => {
    if (
      e.target !== document.querySelector(".to_gmi_dropdown_btn") &&
      e.target !== document.querySelector(".to_gmi_dropdown_btn1")
    ) {
      gmiDropDownHandler();
    }
  });
        ...
        <div className="text-end">
          <div className="d-flex v-center1 flex-end cursur-pointer">
            <img
              src={selectedCoinImg}
              width={22}
              className="mr-10 to_gmi_dropdown_btn"
              onClick={() => setOpenDropDown(!openDropDown)}
            />
            <div
              className="font-20 bold-500 color-white to_gmi_dropdown_btn1"
              onClick={() => setOpenDropDown(!openDropDown)}
            >
              {selectedCoin}
            </div>
