...
export default function Dashboard() {
  ...
  const addReferralButtonDisabledReason = useMemo(() => {
    ...
    const hasReferrer = userRefsQuery.data && Array.isArray(userRefsQuery.data) && userRefsQuery.data.length ? userRefsQuery.data[1] : false
            ...
            <div className='mobile-show'>
              <Skeleton width='400px' loading={userRefsQuery.isLoading}>
                <div className='mt-4 my-2 d-flex flex-column' style={{ gap: '8px' }}>
                  <AsyncButton className='gloop-btn-primary font-16 bold-700 radius-8 bg-green border-green color-dark p-10 min-w-200' disabledreason={addReferralButtonDisabledReason} onClick={handleOpenReferral}>Add Referrer</AsyncButton>
                  <AsyncButton className='gloop-btn-primary font-16 bold-700 radius-8 bg-green border-green color-dark p-10 min-w-200' disabledreason={"Coming Soon"}>Claim Points</AsyncButton>
                </div>
              </Skeleton>
            </div>
                ...
                <div className="d-flex space-between" style={{ gap: '16px' }}>
                  <div className='d-flex flex-column space-between flex-grow-1'>
                    ...
                    <Skeleton loading={userDetailsQuery.isLoading || !userDetailsQuery.data?.lendingUSDCPoints}>
                      <div className='font-20 bold-700 color-white px-2 pb-2'>{truncateAmount(userDetailsQuery.data?.lendingUSDCPoints || '')}</div>
                    </Skeleton>
                  </div>
                  <div className='d-flex flex-column space-between flex-grow-1'>
                    <div className='p-2'>
                    ...
                    <Skeleton loading={userDetailsQuery.isLoading || !userDetailsQuery.data?.borrowingUSDCPoints}>
                      <div className='font-20 bold-700 color-white px-2 pb-2'>{truncateAmount(userDetailsQuery.data?.borrowingUSDCPoints || '')}</div>
                    </Skeleton>
                  </div>
                  <div className='d-flex flex-column space-between flex-grow-1'>
                    <div className='p-2'>
                    ...
                    <Skeleton loading={userDetailsQuery.isLoading || !userDetailsQuery.data?.referralBoost}>
                      <div className='font-20 bold-700 color-white px-2 pb-2'>{truncateAmount(parseInt(userDetailsQuery.data?.referralBoost || 0), 2)}%</div>
                    </Skeleton>
                  </div>
                  <div className='d-flex flex-column space-between flex-grow-1'>
                    <div className='p-2'>
                    ...
                    <Skeleton loading={userDetailsQuery.isLoading}>
                      <div className='font-20 bold-700 color-white px-2 pb-2'>{truncateAmount(userDetailsQuery.data?.usersReferred)}</div>
                    </Skeleton>
                  </div>
                  <div className='d-flex flex-column space-between flex-grow-1'>
                    <div className='p-2'>
                    ...
                    <Skeleton loading={userDetailsQuery.isLoading || !userDetailsQuery.data?.totalEarnedPoints}>
                      <div className='font-20 bold-700 color-green px-2 pb-2'>{truncateAmount(userDetailsQuery.data?.totalEarnedPoints || '')}</div>
                    </Skeleton>
                  </div>
                  <div className='d-flex flex-column space-between flex-grow-1'>
                    <div className='p-2'>
                    ...
                    <Skeleton loading={userDetailsQuery.isLoading}>
                      <div className='font-20 bold-700 color-green px-2 pb-2'>{userDetailsQuery.data?.rank ? `#${userDetailsQuery.data?.rank}` : "-"}</div>
                    </Skeleton>
                  </div>
                </div>
              </div>
