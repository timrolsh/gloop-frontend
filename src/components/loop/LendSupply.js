...
export default function LendSupply() {
  ...
  const handleDeposit = () => {
    ...
    if (!amount)
      return new ValidationException('Fill Deposit Amount First')
  ...
  const buttonDisabledReason = useMemo(() => {
    ...
    if (!amount)
      return 'Fill Deposit Amount First'

    if (createBigNumber(amount.toString()).gt(maxSupply.toString()))
  ...
  const newSuppliedValue = useMemo(() => {
    ...
    return createBigNumber(selectedToken.userPoolBalance).plus(createBigNumber(amount || '0').mul(selectedToken.price)).toString()
  ...
  return (
    <div className='marklendet-supply'>
      ...
      <div className='mt-4'>
        <div className='loop-input-title'>
          ...
          <Skeleton loading={balanceQuery.isLoading || balanceQuery.isFetching} width='100px'>
            <span>{`Wallet Balance: ${truncateAmount(balanceQuery?.data?.toString())} ${selectedToken?.name || ''}`}</span>
          </Skeleton>
        </div>
        {
          selectedToken
            ? <>
              <Skeleton loading={balanceQuery?.isLoading || balanceQuery?.isFetching || configurationsQuery.isLoading || configurationsQuery.isFetching}>
            ...
            <div className='information-detail-items'>
              ...
              <Skeleton loading={selectedToken.supplyApy === env.EMPTY_VALUE}>
                <div className='d-flex justify-content-between'>
                  ...
                  <span className='detail-value'>{truncateAmount(selectedToken.supplyApy, 2)}% </span>
                </div>
              </Skeleton>
            </div>
          </div>
      ...
      <AsyncButton onClick={handleDeposit} loading={buttonLoading} disabledreason={buttonDisabledReason} className='mt-4'>Supply</AsyncButton>
    </div >
  )
}