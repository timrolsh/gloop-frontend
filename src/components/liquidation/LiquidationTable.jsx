        ...
        <div className="radius-8 bg-trans p-4 ">
            <div className='desktop-show'>
                <Table borderless className='gmi_basket_table'>
                    ...
                    <tbody>
                        <tr><td colSpan={8}><hr className='hr-3 border-dark-green m-0' /></td></tr>
                        {
                            !isLoading && !positions.length
                                ? <tr><td className='color-white text-center' colSpan={8}>
                                    <span>{isError ? 'Failed To Fetch Liquidation Data' : 'No Liquidation Data Available'}</span>
                                </td></tr>
                                : positions.length && positions.map((position, index) => (
                                    isLoading || isPlaceholderData
                                        ? <tr key={index}><td colSpan={8}>
            ...
            <div className='mobile-show'>
                {
                    !isLoading && !positions.length
                        ? <span className='color-white text-center w-100 d-block'>{isError ? 'Failed To Fetch Liquidation Data' : 'No Liquidation Data Available'}</span>
                        : positions.length && positions.map((position, index) => (
