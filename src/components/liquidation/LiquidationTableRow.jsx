...
export default function LiquidationTableRow({ position, index, card = false }) {
  ...
  const userTotalCollateralValueQuery = useGetUserCollateralValue({ walletAddress: position.walletAddress })
  ...
  const HFColorClass = useMemo(() => {
    ...
    else if (bigRatio.gte(100) && bigRatio.lte(125))
      return 'color-orange'
    ...
    <>
      {
        card
          ? <div className='d-flex flex-column'>
            ...
            <Skeleton loading={userTotalCollateralValueQuery.isLoading}>
              <div className='d-flex justify-content-between'>
                ...
                <div className='color-white font-14 bold-600 py-2' title={`$${userTotalCollateralValueQuery.data}`}>${truncateAmount(userTotalCollateralValueQuery.data, 2)}</div>
              </div>
            </Skeleton>
            ...
            <div className='d-flex justify-content-between'>
              ...
              <div className={`${HFColorClass} font-14 bold-600 py-2`}>{truncateAmount(position.healthFactor, 0)}%</div>
            </div>

            <div className='d-flex justify-content-between'>
              <div className='color-gray font-14 bold-300 py-2'>Liquidation Bonus</div>
              <div className={`color-white font-14 bold-600 py-2`} title={`${liquidationBonusQuery.data}%`}>{truncateAmount(liquidationBonusQuery.data, 2)}%</div>
            </div>

            <div className='d-flex justify-content-between'>
              <div className='color-gray font-14 bold-300 py-2'>Liquidate</div>
              <div className='color-white font-16 bold-600'><AsyncButton disabledreason={liquidateButtonDisabledReason} className={`${liquidateButtonDisabledReason ? 'bg-white' : ''} p-6-12`} onClick={handleLiquidate}>Liquidate</AsyncButton></div>
            </div>
            <div>
          ...
          : <tr className='py-3'>
            ...
            <Skeleton loading={userTotalCollateralValueQuery.isLoading}>
              <td className='color-white font-16 bold-700' title={`$${userTotalCollateralValueQuery.data}`}>${truncateAmount(userTotalCollateralValueQuery.data, 2)}</td>
            </Skeleton>
            ...
            <td className={`${HFColorClass} font-16 bold-700`}>{truncateAmount(position.healthFactor, 0)}%</td>
            {/* </Skeleton> */}
            <Skeleton loading={liquidationBonusQuery.isLoading}>
              <td className={`color-white font-16 bold-700`} title={`${liquidationBonusQuery.data}%`}>{truncateAmount(liquidationBonusQuery.data, 2)}%</td>
            </Skeleton>
            <td className='color-white font-16 bold-700'><AsyncButton disabledreason={liquidateButtonDisabledReason} className={`${liquidateButtonDisabledReason ? 'bg-white' : ''} p-6-12`} onClick={handleLiquidate}>Liquidate</AsyncButton></td>
          </tr>
      }
      {liquidateModalVisible && <LiquidateUserModal position={position} onClose={() => setLiquidateModalVisible(false)} />}
    </>
