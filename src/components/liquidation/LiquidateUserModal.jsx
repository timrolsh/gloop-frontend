...
export default function LiquidateUserModal({ position, onClose = () => { } }) {
  ...
  const handleLiquidate = () => {
    ...
    liquidate({ borrowedAssetAddress: position.borrowedAssetAddress, repayAmount: amount, borrowerAddress: position.walletAddress }, {
      onSettled: () => {
  ...
  const buttonDisabledReason = useMemo(() => {
    ...
    if (createBigNumber(amount).gt(balanceQuery.data))
      ...
      <Modal.Body className='p-4 radius-8 bg-trans1 border-dark w-100'>
        ...
        <div className='radius-8 border-2 border-dark-green p-3 mt-4'>
          <div className='d-flex space-between v-center'>
            ...
            <div className='radius-8 bg-trans px-3 py-2 font-14 bold-700 color-white cursur-pointer' onClick={() => setDetailsVisible(!detailsVisible)}>{detailsVisible ? 'Hide' : 'Show'}</div>
          </div>
        ...
        <div className='d-flex v-center mt-4 mb-2'>
          ...
          <div className='w-100 d-flex justify-content-between align-items-center'>
            ...
            <Skeleton loading={balanceQuery.isLoading || balanceQuery.isFetching} width='100px' height='20px'>
              <span className='font-14 bold-400 color-gray'>{`Balance: ${truncateAmount(balanceQuery?.data?.toString())} ${token.name || ''}`}</span>
            </Skeleton>
          </div>
        </div>


        <Skeleton loading={!token || balanceQuery.isLoading || balanceQuery.isFetching || balanceQuery.data === env.EMPTY_VALUE}>
            ...
            <div className='w-95-100 d-flex v-center my-1 mr-10'>
              {
                token
                  ? <>
                    <img src={token?.image} width={29} className='mr-10' /><PriceInput amount={amount} setAmount={setAmount} /> <span className='font-18 bold-400 color-gray mr-10'> {token?.name || ''}</span>
                  </>
                  : null
              }
            </div>
        ...
        <div className='mt-2'>
          <AsyncButton onClick={handleLiquidate} loading={buttonLoading} disabledreason={buttonDisabledReason} className='mt-4'>Liquidate</AsyncButton>
        </div>
      </Modal.Body>
    </Modal >
  )
