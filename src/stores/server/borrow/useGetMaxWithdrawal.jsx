...
const useGetMaxWithdrawal = ({ token, walletAddress, enabled = true }) => {
  ...
  const getData = async () => {
    ...
    const remainingBorrowCapacity = createBigNumber(maxBorrowableValue).minus(createBigNumber(userBorrowBalance).mul(usdcPrice).div(1e6))

    if (remainingBorrowCapacity.lte(0))
      return 0 // User cannot withdraw any collateral without dropping health factor below 1
    ...
    return BigNumber.min(collateralWithdrawAmount, formatUnits(userCollateralBalance, token.decimals)).toString()
  }
