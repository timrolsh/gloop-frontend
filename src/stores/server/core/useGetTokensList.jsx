...
const useGetTokensList = ({ enabled = true }) => {
  ...
  const fetchBorrowFields = async (token) => {
    ...
    try {
      ...
      token.utilizationRate = (await getUtilizationRate(token, availableLiquidity, totalBorrows)).toString()
    } catch (error) {
    }
  ...
  const fetchDexFields = async (token) => {
    ...
    try {
      ...
      token.dexTotalControlledValue = formatUnits(totalControlledValue.toString(), token.decimals).toString()
    } catch (error) {
    }
