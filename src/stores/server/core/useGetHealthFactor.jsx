...
const useGetHealthFactor = ({ token, amount, userWalletAddress, enabled = true, formatEther = true }) => {
  ...
  const getData = async () => {
    ...
    if (createBigNumber(totalCollateralQuery.data).gt(0) && createBigNumber(totalBorrowsQuery.data).lte(0) && createBigNumber(amount).equals('0'))
      return Infinity
  ...
  return useQuery({
    queryKey: [queries.GET_HEALTH_FACTOR, token, effectiveWalletAddress, debouncedAmount, formatEther, totalBorrowsQuery, totalCollateralQuery],
    queryFn: getData,
    enabled: enabled
  })
}
