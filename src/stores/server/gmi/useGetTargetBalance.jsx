...
const useGetTargetBalance = ({
  ...
  const getData = async () => {
    if (!token.dexTotalControlledValue) return env.EMPTY_VALUE;

    const parsedDexTotalControlledValue = parseUnits(token.dexTotalControlledValue, token.decimals);

    const targetAmountInWei = await getTargetBalance(
      token,
      withdrawal,
      parsedDexTotalControlledValue,
      roundUp
    );
