...
const getConfigurations = async (token) => {
  ...
  try {
    ...
    throw new Web3Exception(`Failed to Fetch Configurations for ${token.name} Token`, { token, error })
  }
}
...
const getUserAssetBalance = async (token, userWalletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting User Balance of ${token.name} Failed: ${error.shortMessage || 'Unknown Reason!'}`, { token, userWalletAddress, error })
  }
}
...
const getPoolAssetBalance = async (token, userWalletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting Pool Balance of ${token.name} Failed: ${error.shortMessage || 'Unknown Reason!'}`, { token, userWalletAddress, error })
  }
}
...
const getReserveFactorMantissa = async () => {
  try {
    ...
    throw new Web3Exception(`Getting Reserve Factor Failed: ${error.shortMessage || 'Unknown Reason!'}`, { error })
  }
}
...
const calculateHealthFactor = async (token, amount, userWalletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Calculating ${token.name} Health Factor Failed: ${error.shortMessage || 'Unknown Reason!'}`, { token, amount, userWalletAddress, error })
  }
}
...
const calculateHFAfterCollChange = async (token, userWalletAddress, collAmount, collIncrease) => {
  ...
  try {
    ...
    throw new Web3Exception(`Calculating ${token.name} HF After CollChange Failed: ${error.shortMessage || 'Unknown Reason!'}`, { token, userWalletAddress, collAmount, collIncrease, error })
  }
}
...
const addReferrer = async (userWalletAddress) => {
  ...
  try {
    ...
    toastDismiss(toastId)
    throw new Web3Exception(`Referrer Failed: ${error.shortMessage || 'Uknown Reason!'}`, { userWalletAddress, error }, { sendToast: true })
  }
}
...
const getUserRefs = async (userWalletAddress) => {
  try {
    ...
    throw new Web3Exception(`Gettings User Refs Failed: ${error.shortMessage || 'Uknown Reason!'}`, { userWalletAddress, error })
  }
}
...
const getGloopStakersYieldFactor = async () => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting Gloop Stakers Yield Factor Failed: ${error.shortMessage || 'Unknown Reason!'}`, { error })
  }
}
...
const getUtilizationRate = async (token, availableLiquidity, totalBorrows) => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting ${token.name} Utilization Rate Failed`, { token, availableLiquidity, totalBorrows, error, })
  }
}
