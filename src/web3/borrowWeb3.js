...
const getUserBorrowBalance = async (token, userWalletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting User Borrows Balance of ${token.name} Failed`, { token, userWalletAddress, error })
  }
}
...
const getUserCollateralValue = async (userWalletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting User Total Collateral Value Failed`, { userWalletAddress, error })
  }
}
...
const getCollateralEnabledStatus = async (token, userWalletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Getting ${token.name} Collateral Status Failed`, { token, userWalletAddress, error })
  }
}
...
const disableCollateral = async (token) => {
  ...
  try {
    ...
    toastDismiss(toastId)
    throw new Web3Exception(`Disabling ${token.name} Collateral Failed: ${error.shortMessage}`, { token, error }, { sendToast: true })
  }
...
const enableCollateral = async (token) => {
  ...
  try {
    ...
    toastDismiss(toastId)
    throw new Web3Exception(`Enabling ${token.name} Collateral Failed: ${error.shortMessage}`, { token, error }, { sendToast: true })
  }
...
const getMaxBorrowableValue = async (walletAddress) => {
  ...
  try {
    ...
    throw new Web3Exception(`Failed To Get Max BorrowableValue`, { error, walletAddress }, { sendToast: true })
  }
}
...
const borrow = async (amount, token) => {
  ...
  try {
    ...
    if (error instanceof ContractFunctionExecutionError && error.shortMessage.includes('0xfe9cceec'))
...
const repay = async (amount, token) => {
  ...
  try {
    ...
    toastDismiss(toastId)
    throw new Web3Exception(`Repay Failed: ${error.shortMessage || 'Uknown Reason!'}`, { amount, token, error }, { sendToast: true })
  }
}
